...
import { CornerShelf } from "@/components/corner-shelf";
...
import { addGoal, listSupportedBooks, loadBook, signOut } from "@/lib/data-store";
...
export function BookView({ productionMode }: { productionMode: boolean }) {
  ...
  const refresh = useCallback(() => loadBook().then(async (data) => {
    if (data) setBook(data);
    // Someone who only supports other people has no book of their own to open.
    else router.replace((await listSupportedBooks()).length ? "/supporting" : "/onboarding");
  }), [router]);
